...
- **Places are checked against the PSA's own list.** `lib/data/psgc.ts` bundles the 81 provinces (+ Metro Manila) and 1,634 cities/municipalities; `lib/data/psgc-barangays.ts` bundles all 42,046 barangays keyed to their city. Validation therefore costs nothing per order and works if the source API is down. Both files are regenerated by `npx tsx scripts/build-psgc.ts`. A city or province that doesn't exist, is misspelled, or is paired with the wrong province raises a warning naming the likely correction, and so does a **barangay that isn't in that city** — couriers sort on the barangay, so a wrong one is a returned parcel. A city box holding a whole address ("casacon roseller Rt lim Zamboanga sibugay") has its province peeled off and the municipality matched inside the remainder, words-in-order, scoped to that province. Warnings **block saving** — Create order, Save form fields and the Customer card all refuse while a place is flagged. Each carries a one-click **Use "X"** applying a *patch*, so one button can set the city, the province and the barangay together when the customer wrote them in one box; a name shared by several provinces offers one button per province. A deliberate "keep what the customer wrote" tick-box is the only way past. The panel carries a **Copy message for the customer** button producing a short Taglish confirmation. The document's own place fields are found from its form schema (`documentPlacePair`), so birth, CENOMAR, marriage and death are each checked on their own keys. **All four parts of a delivery address — address line, barangay, city, province — are required** to create an order as *details received* (a new-inquiry stub is exempt) and on the public order form; enforced in `createOrder` as well as the UI. The lists stay server-side; the browser asks through the `checkPlaces` action.
